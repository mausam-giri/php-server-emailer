...
/**
 * smtp - PHP email creation and transport class.
 * PHP Version 5.5.
 *
 * @see https://github.com/PHPMailer/PHPMailer/ The smtp GitHub project
 *
...
class PHPMailer
{
     ...
     * If empty, smtp attempts to find one with, in order,
     ...
     * Default is $Hostname. If $Hostname is empty, smtp attempts to find
    ...
    /**
     * An implementation of the smtp OAuthTokenProvider interface.
     *
     ...
     * By default smtp will use `echo` if run from a `cli` or `cli-server` SAPI, `html` otherwise.
     ...
     * @var bool
     *
     * @deprecated 6.0.0 smtp isn't a mailing list manager!
     */
     ...
     * Options: An empty string for smtp default, whitespace/null for none, or a string to use.
     *
     * @var string|null
     */
    public $XMailer = '';
    ...
    /**
     * Add an address to one of the recipient arrays or to the ReplyTo array. Because smtp
     * can't validate addresses with an IDN without knowing the smtp::$CharSet (that can still
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        //Enqueue addresses with IDN until we know the smtp::$CharSet.
     ...
     *
     * ```php
     * smtp::validateAddress('user@example.com', function($address) {
     *     return (strpos($address, '@') !== false);
     * });
     * ```
     *
     * You can also set the smtp::$validator static to a callable, allowing built-in methods to use your validator.
     *
     ...
     * Important: Address must be passed in same encoding as currently set in smtp::$CharSet.
    ...
    public function createHeader()
    {
        ...
        if ('' === $this->XMailer) {
            ...
            $result .= $this->headerLine(
                'X-Mailer',
                'smtp ' . self::VERSION . ' (https://github.com/smtp/smtp)'
            );
     ...
     * smtp only supports some preset message types, not arbitrary MIME structures.
     */
     ...
     * Explicitly *does not* support passing URLs; smtp is not an HTTP client.
     ...
     *                            smtp will attempt to set a MIME type for the attachment.
    ...
    protected function lang($key)
    {
        ...
        if (array_key_exists($key, $this->language)) {
            if ('smtp_connect_failed' === $key) {
                ...
                //but it's usually not smtp's fault.
                return $this->language[$key] . ' https://github.com/smtp/smtp/wiki/Troubleshooting';
            }
